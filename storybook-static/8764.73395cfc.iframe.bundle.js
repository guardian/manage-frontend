/*! For license information please see 8764.73395cfc.iframe.bundle.js.LICENSE.txt */
	...
	[8764],
	{
		...
		'./node_modules/buffer/index.js': (
			...
			function from(value, encodingOrOffset, length) {
				...
				var b = (function fromObject(obj) {
					if (Buffer.isBuffer(obj)) {
						...
						return (
							0 === buf.length || obj.copy(buf, 0, 0, len), buf
						);
					}
			...
			function fromArrayLike(array) {
				for (
					var length =
							array.length < 0 ? 0 : 0 | checked(array.length),
						buf = createBuffer(length),
						i = 0;
					i < length;
					i += 1
			...
			function hexWrite(buf, string, offset, length) {
				...
				length
					? (length = Number(length)) > remaining &&
					  (length = remaining)
					: (length = remaining);
			...
			function blitBuffer(src, dst, offset, length) {
				for (
					var i = 0;
					i < length &&
					!(i + offset >= dst.length || i >= src.length);
					++i
				)
					dst[i + offset] = src[i];
				return i;
