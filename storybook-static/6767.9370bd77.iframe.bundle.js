			...
			(cs.get = function (string) {
						...
						if (
							...
							return (match = string.match(/^(\w+)$/))
								? 'transparent' === match[1]
									...
									? (((rgb = colorNames[match[1]])[3] = 1),
									  rgb)
									: null
								: null;
						for (i = 0; i < 3; i++)
							rgb[i] = Math.round(
								2.55 * parseFloat(match[i + 1]),
							);
						match[4] &&
							(match[5]
								? (rgb[3] = 0.01 * parseFloat(match[4]))
				...
				(cs.to.hex = function () {
					...
					return (
						...
						hexDouble(rgba[2]) +
						(rgba[3] < 1
							? hexDouble(Math.round(255 * rgba[3]))
							: '')
					);
				}),
				(cs.to.rgb = function () {
		...
		'./node_modules/color/index.js': (
			...
			function Color(obj, model) {
				...
				if (void 0 === obj)
					...
					'alpha' in obj &&
						...
						(this.valpha =
							'number' == typeof obj.alpha ? obj.alpha : 0));
			...
			(Color.prototype = {
				...
				object: function () {
					...
					return (
						1 !== this.valpha && (result.alpha = this.valpha),
						result
					);
				},
				unitArray: function () {
				...
				isDark: function () {
					var rgb = this.rgb().color;
					return (
						(299 * rgb[0] + 587 * rgb[1] + 114 * rgb[2]) / 1e3 < 128
					);
				},
				isLight: function () {
					return !this.isDark();
		...
		'./node_modules/simple-swizzle/index.js': (
			...
			var isArrayish = __webpack_require__(
				...
				swizzle = (module.exports = function swizzle(args) {
					for (
						var results = [], i = 0, len = args.length;
						i < len;
						i++
					) {
						var arg = args[i];
