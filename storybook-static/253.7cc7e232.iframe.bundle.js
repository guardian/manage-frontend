...
(self.webpackChunkmanage_frontend =
	...
	[253],
	{
		'./node_modules/msw/lib/core/HttpResponse.mjs': (
			...
			var _utils_HttpResponse_decorators_mjs__WEBPACK_IMPORTED_MODULE_0__ =
				__webpack_require__(
					'./node_modules/msw/lib/core/utils/HttpResponse/decorators.mjs',
				);
			class HttpResponse extends Response {
				constructor(body, init) {
					const responseInit = (0,
					_utils_HttpResponse_decorators_mjs__WEBPACK_IMPORTED_MODULE_0__.p)(
						init,
					);
					super(body, responseInit),
						(0,
						_utils_HttpResponse_decorators_mjs__WEBPACK_IMPORTED_MODULE_0__._A)(
							this,
							responseInit,
						);
				}
				static text(body, init) {
					...
					return (
						responseInit.headers.has('Content-Type') ||
							responseInit.headers.set(
								'Content-Type',
								'text/plain',
							),
						responseInit.headers.has('Content-Length') ||
							responseInit.headers.set(
								'Content-Length',
								body ? new Blob([body]).size.toString() : '0',
							),
						new HttpResponse(body, responseInit)
					);
				}
					...
					return (
						responseInit.headers.has('Content-Length') ||
							responseInit.headers.set(
								'Content-Length',
								responseText
									? new Blob([responseText]).size.toString()
									: '0',
							),
						new HttpResponse(responseText, responseInit)
					);
				}
		...
		'./node_modules/msw/lib/core/http.mjs': (
			...
			__webpack_require__.d(__webpack_exports__, { d: () => http });
					...
					var tokens = (function lexer(str) {
						...
						safePattern = function (prefix) {
							...
							return !prevText ||
								(function (value) {
									for (
										var _i = 0, delimiter_1 = delimiter;
										_i < delimiter_1.length;
										_i++
									) {
										var char = delimiter_1[_i];
										if (value.indexOf(char) > -1) return !0;
									}
									return !1;
								})(prevText)
			...
			function match(str, options) {
				var keys = [];
				return (function regexpToFunction(re, keys, options) {
					void 0 === options && (options = {});
				...
				return (function tokensToRegexp(tokens, keys, options) {
					...
					for (
						var _a = options.strict,
							...
							delimiter = void 0 === _e ? '/#?' : _e,
							_f = options.endsWith,
							endsWithRe = '['.concat(
								escapeString(void 0 === _f ? '' : _f),
								']|$',
							),
							delimiterRe = '['.concat(
								escapeString(delimiter),
						...
						if ('string' == typeof token)
							...
							if (token.pattern)
								if (
									(keys && keys.push(token), prefix || suffix)
								)
									if (
										'+' === token.modifier ||
										'*' === token.modifier
					...
					if (end)
						strict || (route += ''.concat(delimiterRe, '?')),
			...
			function cleanUrl(path) {
				return path.endsWith('?')
					? path
					: path.replace(REDUNDANT_CHARACTERS_EXP, '');
			}
			function normalizePath(path, baseUrl) {
				if (path instanceof RegExp) return path;
				const maybeAbsoluteUrl = (function getAbsoluteUrl(
					path,
					baseUrl,
				) {
					if (
				...
				const normalizedPath = normalizePath(path, baseUrl),
					cleanPath =
						'string' == typeof normalizedPath
							...
							: normalizedPath,
					cleanUrl = (function getCleanUrl(url, isAbsolute = !0) {
						return [isAbsolute && url.origin, url.pathname]
							.filter(Boolean)
							.join('');
					})(url),
					result = match(cleanPath, { decode: decodeURIComponent })(
						cleanUrl,
					);
				return {
					matches: !1 !== result,
			...
			var cb,
				mod,
				toPublicUrl = __webpack_require__(
					'./node_modules/msw/lib/core/utils/request/toPublicUrl.mjs',
				),
				__create = Object.create,
				index_esm_defProp = Object.defineProperty,
				__getOwnPropDesc = Object.getOwnPropertyDescriptor,
				__getOwnPropNames = Object.getOwnPropertyNames,
				__getProtoOf = Object.getPrototypeOf,
				__hasOwnProp = Object.prototype.hasOwnProperty,
				source_default = ((mod, isNodeMode, target) => (
					...
					((to, from, except, desc) => {
						...
						!isNodeMode && mod && mod.__esModule
							? target
							: index_esm_defProp(target, 'default', {
									value: mod,
									enumerable: !0,
							  }),
						mod,
						...
						'node_modules/cookie/index.js'(exports) {
							(exports.parse = function parse(str, options) {
								...
								(exports.serialize = function serialize(
									...
									if (opt.domain) {
										if (
											!fieldContentRegExp.test(opt.domain)
										)
											throw new TypeError(
				...
				cookieStore = __webpack_require__(
			...
			function getAllRequestCookies(request) {
				const requestCookieHeader = request.headers.get('cookie'),
					cookiesFromHeaders = requestCookieHeader
						? source_default.parse(requestCookieHeader)
						: {},
					cookiesFromDocument = (function getDocumentCookies(
						request,
					) {
						if (
				...
				for (const name in cookiesFromDocument)
					request.headers.append(
						'cookie',
						source_default.serialize(
							name,
							cookiesFromDocument[name],
						),
					);
				const cookiesFromStore = cookieStore.k.getCookiesSync(
						request.url,
					),
					storedCookiesObject = Object.fromEntries(
						cookiesFromStore.map((cookie) => [
							cookie.key,
							cookie.value,
						]),
					);
			...
			var RequestHandler = __webpack_require__(
			...
			class HttpHandler extends RequestHandler.Z {
				constructor(method, path, resolver, options) {
					super({
						info: { header: `${method} ${path}`, path, method },
						resolver,
						options,
					}),
				...
				checkRedundantQueryParameters() {
					...
					const searchParams = (function getSearchParams(path) {
				...
				predicate(args) {
					const hasMatchingMethod = this.matchMethod(
							args.request.method,
						),
						hasMatchingUrl = args.parsedResult.match.matches;
				...
				async log(args) {
					const publicUrl = (0, toPublicUrl.F)(args.request.url),
						loggedRequest = await (async function serializeRequest(
							request,
						) {
						...
						loggedResponse =
							await (async function serializeResponse(response) {
						...
						statusColor = (function getStatusCodeColor(status) {
							return status < 300
								? '#69AB32'
								: status < 400
								? '#F0BB4B'
						...
						devUtils.e.formatMessage(
							`${(function getTimestamp() {
								...
								return [
									...
									.join(':');
							})()} ${args.request.method} ${publicUrl} (%c${
								loggedResponse.status
			...
			function createHttpHandler(method) {
				return (path, resolver, options = {}) =>
					new HttpHandler(method, path, resolver, options);
			}
			const http = {
