  ...
  public render = () => (
    ...
    >
      <p>
        ...
        the number of issues you are going to miss (up to a maximum of 6 per
        year) and estimated credit you will get.
      </p>
      <p>
        The first available date is{" "}
        <strong>{firstAvailableDateMoment.format("dddd D MMMM")}</strong> due to
        printing and delivery schedules, and you can book up to one year ahead.
      </p>
      <p>
        You can select only one schedule at a time. You have{" "}
        <strong>{issuesRemaining}</strong> issues out of {MAX_ISSUES_PER_YEAR}{" "}
      ...
      <DatePicker
        ...
        onSelect={({ start, end }) => {
          ...
          const allowedIssues =
            issuesSelected > issuesRemaining ? issuesRemaining : issuesSelected;
          this.setState({
            selectedRange: range,
